...
function revokeCollectorAgentSiteSessions(state, siteId) {
  ...
  state.agentBySite.delete(key);
  deleteAgentSessionsBySite(key).catch(() => {});
}
...
function issueCollectorAgentSession(state, site, payload = {}, remoteIp = '') {
  ...
  setCollectorAgentPresence(state, session, now);
  persistAgentSession(session).catch(() => {});
  return session;
}
...
async function restoreCollectorAgentSessions(state) {
  return restoreAgentSessions(state.agentSessions, state.agentBySite, Date.now());
}
...
function toolsTerminalHelpLines(state, site, collectorMode = false) {
  ...
  return normalizeToolsTerminalLines([
    ...
    '3) snmp poll',
    `4) tcp ${snmpHost || defaultHost} 161`,
    '5) syslog diag',
    '6) netflow diag',
    '7) netflow top',
    `8) listeners   # verify local ${syslogPort}/UDP+TCP and ${netflowPort}/UDP listeners`
  ]);
}
...
async function runToolsTerminalCommand(state, site, commandText = '') {
  ...
  const { runSnmpGet, formatSysUpTimeTicks } = require('./monitoring');
  ...
  if (command === 'snmp') {
    ...
    if (sub === 'probe' || sub === 'oids') {
      if (!String(snmpCfg.targetHost || '').trim()) {
        return { ok: false, exitCode: 1, lines: ['SNMP targetHost missing in monitor config.'] };
      }
      const { runSnmpOidProbe } = require('./monitoring');
      const results = await runSnmpOidProbe(snmpCfg);
      ...
      return { ok, exitCode: ok ? 0 : 1, lines: normalizeToolsTerminalLines([`OID probe for ${snmpCfg.targetHost}`, ...lines]) };
    }
    if (sub === 'info') {
      if (!String(snmpCfg.targetHost || '').trim()) {
        return { ok: false, exitCode: 1, lines: ['SNMP targetHost missing in monitor config.'] };
      }
      const { runSnmpWalk, formatSysUpTimeTicks: fmtTicks } = require('./monitoring');
      const lines = [`SNMP Device Info — ${snmpCfg.targetHost}`, `version=${snmpCfg.version || '2c'}`];
      // Walk system MIB (sysDescr, sysOID, sysUpTime, sysContact, sysName, sysLocation)
      const SYSTEM_MIB = '1.3.6.1.2.1.1';
      const SYS_OID_NAMES = {
      ...
      try {
        const raw = await runSnmpWalk(snmpCfg, SYSTEM_MIB);
        for (const line of raw.trim().split('\n').filter(Boolean)) {
          const eqIdx = line.indexOf(' ');
          if (eqIdx < 0) { lines.push(line); continue; }
          const oid = line.slice(0, eqIdx).replace(/^\./, '');
          const val = line.slice(eqIdx + 1).trim().replace(/^(STRING|INTEGER|Gauge32|Counter32|OID|Timeticks)[:\s]+/i, '').replace(/^"(.*)"$/, '$1');
          const label = SYS_OID_NAMES[oid] || oid;
          lines.push(`${label}: ${val.slice(0, 200)}`);
        }
      } catch (err) {
        lines.push(`System MIB walk failed: ${String(err?.message || err || 'Unknown error')}`);
      }
      // Interface count from cached metrics
      const ifCount = site?.metrics?.snmp?.interfaceCount;
      const lastPoll = site?.metrics?.snmp?.lastPoll || 'never';
      const lastError = site?.metrics?.snmp?.lastError || '';
      const successCount = site?.metrics?.snmp?.successCount || 0;
      const failCount = site?.metrics?.snmp?.failureCount || 0;
      lines.push('');
      lines.push(`Interfaces in IF-MIB: ${ifCount !== undefined ? ifCount : 'N/A (not yet polled)'}`);
      lines.push(`Last poll: ${lastPoll}  success/fail: ${successCount}/${failCount}`);
      if (lastError) lines.push(`Last error: ${lastError}`);
      lines.push(`Trap receiver: UDP ${require('./constants').SNMP_TRAP_PORT}`);
      return { ok: true, exitCode: 0, lines: normalizeToolsTerminalLines(lines) };
    }
    if (sub === 'walk') {
      const oidToken = sanitizeToolsTerminalHostToken(args[1] || '');
      if (!oidToken) return { ok: false, exitCode: 1, lines: ['Usage: snmp walk <oid>'] };
      if (!String(snmpCfg.targetHost || '').trim()) {
        return { ok: false, exitCode: 1, lines: ['SNMP targetHost missing in monitor config.'] };
      }
      const { runSnmpWalk } = require('./monitoring');
      try {
        const raw = await runSnmpWalk(snmpCfg, oidToken);
        const lines = splitNonEmptyLines(raw).slice(0, 200);
        return { ok: true, exitCode: 0, lines: normalizeToolsTerminalLines([`snmpwalk ${snmpCfg.targetHost} ${oidToken}`, ...lines]) };
      } catch (err) {
        return { ok: false, exitCode: 1, lines: [`snmp walk failed: ${String(err?.message || err || 'Unknown error')}`] };
      }
    }
    return { ok: false, exitCode: 1, lines: ['Unknown SNMP command. Try: snmp poll | snmp diag | snmp probe | snmp info | snmp walk <oid>'] };
  }
  ...
  if (command === 'syslog') {
    ...
    if (sub === 'test') {
      const { updateSyslogMetrics } = require('./monitoring');
      updateSyslogMetrics(state, site, Date.now(), '127.0.0.1');
      return { ok: true, exitCode: 0, lines: normalizeToolsTerminalLines([
        `Synthetic syslog event injected for site "${site?.name || site?.id || 'unknown'}"`,
        'Check the event log — a syslog_ingest entry should appear.',
        `If syslog shows LIVE, the pipeline is functional.`
      ]) };
    }
    return { ok: false, exitCode: 1, lines: ['Unknown SYSLOG command. Try: syslog diag | syslog test'] };
  }
  ...
  if (command === 'tcp') {
    const host = sanitizeToolsTerminalHostToken(args[0] || '');
    const port = parseInt(args[1], 10);
    if (!host || !port || isNaN(port) || port < 1 || port > 65535) {
      return { ok: false, exitCode: 1, lines: ['Usage: tcp <host> <port>'] };
    }
    const net = require('net');
    ...
    const tcpResult = await new Promise((resolve) => {
      ...
      socket.on('timeout', () => {
        socket.destroy();
        resolve({ ok: false, reason: 'Timeout after 5000ms' });
      });
      socket.on('error', (err) => {
        resolve({ ok: false, reason: err.message });
      });
    });
    const tcpLines = tcpResult.ok
      ? [`TCP ${host}:${port} — OPEN (${tcpResult.ms}ms)`]
      : [`TCP ${host}:${port} — CLOSED/FILTERED (${tcpResult.reason})`];
    return { ok: tcpResult.ok, exitCode: tcpResult.ok ? 0 : 1, lines: normalizeToolsTerminalLines(tcpLines) };
  }

  if (command === 'arp') {
    const host = args[0] ? sanitizeToolsTerminalHostToken(args[0]) : null;
    const arpArgs = host ? ['-n', host] : ['-n'];
    const neighArgs = host ? ['neigh', 'show', host] : ['neigh', 'show'];
    const result = await runToolsTerminalExecCandidates([
      { cmd: 'arp', args: arpArgs },
      { cmd: 'ip', args: neighArgs }
    ], TOOLS_TERMINAL_TIMEOUT_MS);
    return { ok: !result.err, exitCode: result.err ? 1 : 0, lines: toolsTerminalLinesFromExec(result, `arp${host ? ' ' + host : ''}`) };
  }

  if (command === 'mib') {
    const oidToken = sanitizeToolsTerminalHostToken(args[0] || '');
    if (!oidToken) return { ok: false, exitCode: 1, lines: ['Usage: mib <oid-or-name>'] };
    const result = await runToolsTerminalExecCandidates([
      { cmd: 'snmptranslate', args: ['-IR', '-On', oidToken] },
      { cmd: 'snmptranslate', args: [oidToken] }
    ], TOOLS_TERMINAL_TIMEOUT_MS);
    if (result.err && String(result.err?.code || '').toUpperCase() === 'ENOENT') {
      return { ok: false, exitCode: 1, lines: ['snmptranslate not found. Install net-snmp-utils or snmp package.'] };
    }
    return { ok: !result.err, exitCode: result.err ? 1 : 0, lines: toolsTerminalLinesFromExec(result, `mib ${oidToken}`) };
  }
