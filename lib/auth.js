...
// JWKS cache: { [issuerUrl]: { keys: [], fetchedAt: number } }
const _jwksCache = new Map();
const JWKS_CACHE_TTL_MS = 3600000; // 1 hour

async function fetchJwks(tenantId) {
  const issuerUrl = `https://login.microsoftonline.com/${encodeURIComponent(tenantId)}/discovery/v2.0/keys`;
...
async function verifyJwt(token, tenantId) {
  const parts = String(token || '').split('.');
  if (parts.length !== 3) return { verified: false, claims: {} };
  try {
    const headerJson = Buffer.from(parts[0], 'base64url').toString('utf8');
    const header = JSON.parse(headerJson);
    const kid = String(header.kid || '');
    const alg = String(header.alg || '').toUpperCase();
    if (!kid || !alg.startsWith('RS')) return { verified: false, claims: decodeJwtPayload(token) };
    const keys = await fetchJwks(tenantId);
    const jwk = keys.find((k) => k.kid === kid);
    if (!jwk) return { verified: false, claims: decodeJwtPayload(token) };
    const pem = jwkToPem(jwk);
    const signInput = `${parts[0]}.${parts[1]}`;
    const signature = Buffer.from(parts[2], 'base64url');
    const verified = crypto.createVerify(alg === 'RS384' ? 'RSA-SHA384' : alg === 'RS512' ? 'RSA-SHA512' : 'RSA-SHA256')
      .update(signInput)
      .verify(pem, signature);
    return { verified, claims: decodeJwtPayload(token) };
  } catch {
    return { verified: false, claims: decodeJwtPayload(token) };
  }
}
