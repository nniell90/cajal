...
// If a _FILE env var points to a Docker secret, read it and set the base var.
function loadDockerSecret(envVar, fileEnvVar) {
  if (process.env[envVar]) return; // explicit value takes precedence
  const secretPath = process.env[fileEnvVar];
  if (!secretPath) return;
  try {
    const value = fs.readFileSync(secretPath, 'utf8').trim();
    if (value) process.env[envVar] = value;
  } catch {
    // Secret file not found — fall through to default
  }
}
loadDockerSecret('CAJAL_DB_PASSWORD', 'CAJAL_DB_PASSWORD_FILE');
...
const SNMP_TRAP_PORT = Number(process.env.CAJAL_SNMP_TRAP_PORT || 1162);
...
const LDAP_FILE = path.join(DATA_DIR, 'ldap.json');
...
// ── Database settings ─────────────────────────────────────────────────────────
const CONFIG_INTEGRITY_KEYS = Object.freeze(['sso', 'runtime', 'apiTokens', 'ssl', 'ldap', 'bootstrap']);
...
const defaultLocationSettings = {
  ...
  internalName: 'Demo Location',
  customerName: '',
  sections: [
    { id: 'internal', name: 'Demo Location', address: '123 Work street', pingMonitors: [
      { id: 'pm-038dce3a23', label: 'Demo Device', target: '192.168.1.100' }
    ] }
  ]
};
...
const STORAGE_TRACKED_FILES = [
  ...
  { name: 'ldap', file: LDAP_FILE, category: 'config' },
