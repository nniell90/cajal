...
const GITHUB_REPO = String(process.env.CAJAL_GITHUB_REPO || 'sentineledge-ai/cajal').trim();
const UPDATE_IMAGE = String(process.env.CAJAL_UPDATE_IMAGE || 'ghcr.io/sentineledge-ai/cajal:latest').trim();
const WATCHTOWER_URL = String(process.env.CAJAL_WATCHTOWER_URL || 'http://cajal-watchtower:8080').trim().replace(/\/$/, '');
...
const DATABASE_SSL_MODE = String(process.env.CAJAL_DATABASE_SSL || 'disable').trim().toLowerCase();
