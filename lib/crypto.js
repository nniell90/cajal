...
function decryptJson(blob) {
  ...
  try {
    ...
    const prevKey = getPreviousCryptoKey();
    if (!prevKey) throw currentKeyErr;
    try {
      return decryptJsonWithKey(blob, prevKey);
    } catch {
      throw currentKeyErr;
    }
  }
...
function encryptBackupPayload(payload, password) {
  ...
  if (pass.length < 6) throw new Error('Backup password must be at least 6 characters');
