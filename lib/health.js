...
async function runMonitorDiagnostics(state, site, protocol, config = {}, options = {}) {
  const { recentFlowCheck, runSnmpGet, formatSysUpTimeTicks, mergeConfig } = require('./monitoring');
  const checks = [];
  ...
  if (protocolName === 'syslog') {
    ...
    addDiagnosticCheck(checks, 'recent_ingest', flow.status, flow.detail);
    addDiagnosticCheck(checks, 'template_state', templateCount > 0 ? 'pass' : 'warn', templateCount > 0 ? `${templateCount} exporter template map(s) loaded` : 'No NetFlow/IPFIX templates seen yet');
