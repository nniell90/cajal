...
async function testLdapConnection({ serverUrl, port, baseDn, adminGroup, monitorGroup, bindDn, bindPassword }) {
  ...
  try {
    ...
    steps[steps.length - 1].status = 'ok';
    steps[steps.length - 1].detail = `Bind succeeded as "${bindDn}"`;
  } catch (err) {
    steps[steps.length - 1].status = 'fail';
  ...
  if (adminGroup) {
    ...
    try {
      ...
      if (!result.found) {
        ...
        steps[steps.length - 1].detail = `Group "${adminGroup}" not found under baseDn "${baseDn}". Searched as AD group (objectClass=group) and posixGroup. Check the group name is exact (case-sensitive in some directories).`;
      } else {
        steps[steps.length - 1].status = 'ok';
  ...
  if (monitorGroup) {
    ...
    try {
      ...
      if (!result.found) {
        steps[steps.length - 1].status = 'warn';
        steps[steps.length - 1].detail = `Group "${monitorGroup}" not found under baseDn "${baseDn}". Searched as AD group and posixGroup. Check the group name is exact.`;
      } else {
        steps[steps.length - 1].status = 'ok';
