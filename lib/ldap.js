...
async function testLdapConnection({ serverUrl, port, baseDn, adminGroup, monitorGroup, bindDn, bindPassword }) {
  ...
  // Step 1: TCP Ping
  addStep('TCP Ping', 'running', `Connecting to ${host}:${port}...`);
  const pingResult = await tcpPing(host, Number(port) || 389, 5000);
  if (pingResult.ok) {
    steps[steps.length - 1].status = 'ok';
    steps[steps.length - 1].detail = `Connected to ${host}:${port} in ${pingResult.ms}ms`;
  } else {
    steps[steps.length - 1].status = 'fail';
    steps[steps.length - 1].detail = `Failed to reach ${host}:${port} — ${pingResult.error}`;
    return { ok: false, steps, users: [] };
  }

  // Step 2: LDAP Bind
  addStep('LDAP Bind', 'running', `Authenticating as ${bindDn}...`);
  let client;
  try {
    client = createLdapClient(serverUrl, port);
    await ldapBind(client, bindDn, bindPassword);
    steps[steps.length - 1].status = 'ok';
    steps[steps.length - 1].detail = `Successfully authenticated as ${bindDn}`;
  } catch (err) {
    steps[steps.length - 1].status = 'fail';
    steps[steps.length - 1].detail = `Bind failed: ${err?.message || String(err)}`;
  ...
  const allUsers = [];
  if (adminGroup) {
    addStep('Read Admin Group', 'running', `Searching for group "${adminGroup}" in ${baseDn}...`);
    try {
      const result = await resolveGroupMembers(client, baseDn, adminGroup);
      if (!result.found) {
        steps[steps.length - 1].status = 'warn';
        steps[steps.length - 1].detail = `Group "${adminGroup}" not found in directory`;
      } else {
        steps[steps.length - 1].status = 'ok';
        steps[steps.length - 1].detail = `Found ${result.members.length} member(s) in "${adminGroup}" (${result.groupDn})`;
      ...
      steps[steps.length - 1].status = 'fail';
      steps[steps.length - 1].detail = `Admin group search failed: ${err?.message || String(err)}`;
    }
  }

  // Step 4: Search monitor group
  if (monitorGroup) {
    addStep('Read Monitor Group', 'running', `Searching for group "${monitorGroup}" in ${baseDn}...`);
    try {
      const result = await resolveGroupMembers(client, baseDn, monitorGroup);
      if (!result.found) {
        steps[steps.length - 1].status = 'warn';
        steps[steps.length - 1].detail = `Group "${monitorGroup}" not found in directory`;
      } else {
        steps[steps.length - 1].status = 'ok';
        steps[steps.length - 1].detail = `Found ${result.members.length} member(s) in "${monitorGroup}" (${result.groupDn})`;
        for (const m of result.members) {
      ...
      steps[steps.length - 1].status = 'fail';
      steps[steps.length - 1].detail = `Monitor group search failed: ${err?.message || String(err)}`;
    }
  }
  ...
  return { ok: true, steps, users: allUsers };
}
