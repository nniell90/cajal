...
function startNetflowCollector(state) {
  ...
  socket.on('message', (msg, rinfo) => {
    ...
    if (!records || !records.length) {
      site.metrics = site.metrics || {};
      site.metrics.netflow = site.metrics.netflow || {};
      site.metrics.netflow.templateCount = templateMap.size;
    ...
    for (const site of state.sites) {
      ...
      try {
        ...
        try {
          ...
          const upCount = withDeltas.filter((i) => i.ifOperStatus === 'up').length;
          const topIf = withDeltas.find((i) => i.totalMbps > 0);
          ifSummary = ` interfaces=${raw.length} up=${upCount} down=${raw.length - upCount}`;
          if (topIf) ifSummary += ` top=${topIf.ifDescr}@${topIf.totalMbps}Mbps`;
        } catch (ifErr) {
          site.metrics.snmp.interfaces = site.metrics.snmp.interfaces || [];
          ifSummary = ` ifwalk_error=${String(ifErr?.message || ifErr || 'unknown').slice(0, 60)}`;
          logDiagnostic(state, {
            level: 'warn',
        ...
        const pollDetail = `uptime=${site.metrics.snmp.uptime} responseMs=${responseMs}${ifSummary}`;
        logSiteEvent(state, site, {
          classId: 322,
          source: 'snmp',
          actor: 'collector',
...
const SNMP_TRAP_OID_VB = '1.3.6.1.6.3.1.1.4.1.0';
const SNMP_SYSUPTIME_OID = '1.3.6.1.2.1.1.3.0';

function safeVbValue(v) {
  if (v === null || v === undefined) return '';
  if (Buffer.isBuffer(v)) return v.toString('hex');
  return String(v).trim();
}

function startSnmpTrapReceiver(state) {
  let snmpLib;
  try {
    snmpLib = require('net-snmp');
  } catch (_) {
    logDiagnostic('snmp.trap', 'net-snmp not available — SNMP trap receiver disabled');
    return;
  }

  function handleTrap(error, notification) {
    if (error) {
      logSystemError('snmp.trap', error);
      return;
    }

    const sourceIp = notification.rinfo?.address || 'unknown';
    const pdu = notification.pdu || {};
    const varbinds = Array.isArray(pdu.varbinds) ? pdu.varbinds : [];
    const community = pdu.community ? String(pdu.community) : '';

    // Match trap source IP to a known site's SNMP target
    const site = state.sites.find((s) => {
      const cfg = s.monitorConfig?.snmp;
      return cfg?.enabled && cfg.targetHost === sourceIp;
    });

    // Extract snmpTrapOID varbind to get the trap type
    const trapOidVb = varbinds.find((v) => v.oid === SNMP_TRAP_OID_VB);
    const trapOid = trapOidVb ? safeVbValue(trapOidVb.value) : null;
    const trapName = (trapOid && TRAP_OID_NAMES[trapOid]) ? TRAP_OID_NAMES[trapOid] : (trapOid || 'generic');

    // Extract uptime if present
    const uptimeVb = varbinds.find((v) => v.oid === SNMP_SYSUPTIME_OID);
    const uptimePart = uptimeVb ? `uptime=${safeVbValue(uptimeVb.value)}` : '';

    // All other varbinds as detail
    const otherVbs = varbinds
      .filter((v) => v.oid !== SNMP_TRAP_OID_VB && v.oid !== SNMP_SYSUPTIME_OID)
      .map((v) => `${v.oid}=${safeVbValue(v.value)}`)
      .join(' ');
    ...
    const siteLabel = site ? site.name : sourceIp;
    const message = `SNMP trap: ${trapName} from ${siteLabel}`;

    // linkDown / coldStart / authFailure are alerts; linkUp / warmStart are recoveries
    const isAlert = trapName === 'linkDown' || trapName === 'coldStart' || trapName === 'authenticationFailure';
    const classId = isAlert ? 402 : 322;

    logDiagnostic('snmp.trap', message, { detail });

    if (site) {
      logSiteEvent(state, site, { classId, source: 'snmp', actor: 'collector', action: 'snmp_trap', message, detail });
    } else {
      logEvent(state, { classId, source: 'snmp', actor: 'collector', action: 'snmp_trap', message, detail });
    }
  }

  try {
    snmpLib.createReceiver({ port: SNMP_TRAP_PORT, disableAuthorization: true }, handleTrap);
    logDiagnostic('snmp.trap', `SNMP trap receiver listening on UDP ${SNMP_TRAP_PORT}`);
  } catch (err) {
    logSystemError('snmp.trap.start', err);
  }
}
