  ...
  return new Promise((resolve, reject) => {
    ...
    if (config.version === '3') {
      ...
      if (config.privPassword) {
        ...
        args = ['-v3', '-l', 'authPriv', '-u', config.authUser, '-a', authProto, '-A', config.authPassword || '', '-x', privProto, '-X', config.privPassword, host, '1.3.6.1.2.1.1.3.0'];
      } else {
        args = ['-v3', '-l', 'authNoPriv', '-u', config.authUser, '-A', config.authPassword || '', host, '1.3.6.1.2.1.1.3.0'];
      }
    ...
    execFile('snmpget', args, { timeout: 12000 }, (err, stdout, stderr) => {
...
function startSyslogCollectors(state) {
  ...
  const tcp = net.createServer((socket) => {
    socket.on('data', (chunk) => {
      ...
      const preview = formatPacketPreview(chunk);
      const lines = parseLinesFromBuffer(chunk);
      const site = matchSiteBySourceIp(state, 'syslog', sourceIp);
      if (!site) {
        handleUnmatchedSyslogPacket(state, {
          sourceIp,
...
function startSnmpPoller(state) {
  ...
  poll().catch((err) => logSystemError('poller.snmp.initial', err));
  setInterval(() => {
    poll().catch((err) => logSystemError('poller.snmp.interval', err));
  }, SNMP_POLL_INTERVAL_MS);
}
