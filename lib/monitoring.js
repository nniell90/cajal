...
const { markSiteDirty, normalizeHeartbeatTarget, isLinkMarkedNone, resolveSiteGatewayIp, applyFlowStatus } = require('./sites');
...
function runSnmpGet(config) {
  return new Promise((resolve, reject) => {
    ...
    if (!host) return reject(new Error('targetHost missing'));

    // Use a temporary snmp.conf to keep credentials out of the process table.
    // snmpget reads credentials from SNMPCONFPATH when available.
    let tmpConfPath = '';
    let args;
    const execOpts = { timeout: 12000 };

    if (config.version === '3') {
      if (!config.authUser) return reject(new Error('SNMPv3 authUser missing'));
      ...
      if (config.privPassword) {
        ...
        args = ['-v3', '-l', 'authPriv', host, '1.3.6.1.2.1.1.3.0'];
      } else {
        confLines.push(`defSecurityLevel authNoPriv`);
        args = ['-v3', '-l', 'authNoPriv', host, '1.3.6.1.2.1.1.3.0'];
      }
      ...
      args = ['-v1', '-c', config.communityString || '', host, '1.3.6.1.2.1.1.3.0'];
    } else {
      args = ['-v2c', '-c', config.communityString || '', host, '1.3.6.1.2.1.1.3.0'];
    }

    execFile('snmpget', args, execOpts, (err, stdout, stderr) => {
      ...
      if (!match) return reject(new Error('Unexpected snmpget output'));
...
function runSnmpWalk(config, oid) {
  return new Promise((resolve, reject) => {
    ...
    let tmpConfPath = '';
    let args;
    const execOpts = { timeout: 15000, maxBuffer: 512 * 1024 };
    ...
    if (config.version === '3') {
      ...
      if (config.privPassword) {
        ...
        confLines.push(`defPrivType ${privProto}`);
        confLines.push(`defPrivPassphrase ${config.privPassword}`);
        args = ['-v3', '-l', 'authPriv', '-Oqn', host, oid];
      } else {
        confLines.push(`defSecurityLevel authNoPriv`);
        args = ['-v3', '-l', 'authNoPriv', '-Oqn', host, oid];
      }
      tmpConfPath = path.join(os.tmpdir(), `.cajal-snmp-${Date.now()}-${Math.random().toString(36).slice(2)}`);
      fs.mkdirSync(tmpConfPath, { recursive: true, mode: 0o700 });
      fs.writeFileSync(path.join(tmpConfPath, 'snmp.conf'), confLines.join('\n'), { mode: 0o600 });
      ...
      args = ['-v1', '-c', config.communityString || '', '-Oqn', host, oid];
    } else {
      args = ['-v2c', '-c', config.communityString || '', '-Oqn', host, oid];
    }

    execFile('snmpwalk', args, execOpts, (err, stdout, stderr) => {
      if (tmpConfPath) {
        try { fs.rmSync(tmpConfPath, { recursive: true, force: true }); } catch (_) {}
      }
      if (err) return reject(new Error(stderr.trim() || err.message));
  ...
  const poll = async () => {
    ...
    for (const site of state.sites) {
      ...
      try {
        ...
        site.metrics = site.metrics || {};
        site.metrics.snmp = site.metrics.snmp || {};
        site.metrics.snmp.uptime = formatSysUpTimeTicks(ticks);
