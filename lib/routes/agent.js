...
function handleAgent({ req, res, url, state, requestUser, requestContext }) {
    ...
    if (req.method === 'GET' && url.pathname === '/api/agent/windows/download') {
      ...
      // Default: serve .exe (bundled in image, or uploaded via Settings)
      return (async () => {
        let payload = null;
        ...
        if (!payload?.length) {
          return sendJson(res, 404, {
            error: 'Windows .exe package not available on this server. Upload one in Settings > API or use ?format=ps1 for the PowerShell script.'
          });
        }
