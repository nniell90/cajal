...
const { runSnmpGet, runSnmpOidProbe, updateSyslogMetrics, refreshNetflowTopTalkers } = require('../monitoring');
...
function handleSites({ req, res, url, state, requestUser, requestContext }) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/sites') {
    ...
    return readRequestBody(req, {
      ...
      .then(async (body) => {
        ...
        if (name.length > 255) return sendJson(res, 400, { error: 'name must be 255 characters or fewer' });
  ...
  if (req.method === 'POST' && /^\/api\/sites\/[^/]+\/monitors\/snmp\/oid-probe$/.test(url.pathname)) {
    if (!ensureAllowed(requestUser, ['admin'])) return sendJson(res, 403, { error: 'Forbidden' });
    const match = url.pathname.match(/^\/api\/sites\/([^/]+)\/monitors\/snmp\/oid-probe$/);
    const siteId = match?.[1];
    if (!siteId) return sendJson(res, 400, { error: 'Invalid OID probe path' });
    const site = state.sites.find((s) => s.id === siteId);
    if (!site) return sendJson(res, 404, { error: 'Site not found' });

    return readRequestBody(req, { allowedKeys: ['config'] })
      .then(async (body) => {
        const baseCfg = site.monitorConfig?.snmp || {};
        const testCfg = mergeConfig(baseCfg, body?.config || {});
        if (!String(testCfg.targetHost || '').trim()) return sendJson(res, 400, { error: 'SNMP targetHost is required' });
        const results = await runSnmpOidProbe(testCfg);
        return sendJson(res, 200, { ok: true, targetHost: String(testCfg.targetHost || ''), results });
      })
      .catch((err) => sendJson(res, 400, badRequestPayload(err)));
  }
