...
function handleSystem({ req, res, url, state, requestUser, requestContext }) {
    ...
    if (req.method === 'GET' && url.pathname === '/api/system/version/check') {
      ...
      return new Promise((resolve) => {
        ...
        const tokenReq = https.request(tokenUrl, (tokenRes) => {
          ...
          tokenRes.on('end', () => {
            try {
              ...
              tagsReq.on('error', (err) => resolve(sendJson(res, 502, { error: `GHCR API unreachable: ${err.message}` })));
        ...
        tokenReq.on('error', (err) => resolve(sendJson(res, 502, { error: `GHCR token request failed: ${err.message}` })));
