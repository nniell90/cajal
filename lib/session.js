...
function applySecurityHeaders(req, res, url) {
  ...
  if (!res.hasHeader('Content-Security-Policy')) {
    res.setHeader(
      ...
      "default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; object-src 'none'; connect-src 'self'; img-src 'self' https:; script-src 'self'; style-src 'self'; font-src 'self'"
    );
  }
