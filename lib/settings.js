...
const { sanitizeApiTokenSettings, parseCollectorAgentInstallTimestampMs } = require('./tokens');
...
// ── Windows agent package settings ───────────────────────────────────────────
function sanitizeWindowsAgentPackageFileName(value = '') {
  const path = require('path');
  const raw = String(value || '').trim();
  const fallback = 'cajal-windows-agent.exe';
  if (!raw) return fallback;
  const base = path.basename(raw).replace(/[^\w.-]+/g, '_').slice(0, 120);
  if (!base) return fallback;
  if (!base.toLowerCase().endsWith('.exe')) return `${base}.exe`;
  return base;
}

function normalizeWindowsAgentPackageBase64(value = '') {
  const raw = String(value || '').trim();
  if (!raw) return '';
...
function decodeWindowsAgentPackageBuffer(config = {}) {
  const base64 = normalizeWindowsAgentPackageBase64(config?.dataBase64 || '');
  if (!base64) return null;
  try {
    const payload = Buffer.from(base64, 'base64');
    if (!payload.length) return null;
    if (!isWindowsExeBuffer(payload)) return null;
    return payload;
  } catch {
    return null;
  }
...
function normalizeWindowsAgentPackageSettings(config = {}) {
  const source = config && typeof config === 'object' ? config : {};
  const fileName = sanitizeWindowsAgentPackageFileName(source.fileName || '');
  const uploadedAtMs = parseCollectorAgentInstallTimestampMs(source.uploadedAt || '');
  const normalizedBase64 = normalizeWindowsAgentPackageBase64(source.dataBase64 || '');
  let sizeBytes = Number(source.sizeBytes);
  if (!Number.isFinite(sizeBytes) || sizeBytes < 0) sizeBytes = 0;
  if (normalizedBase64) {
    try {
      const payload = Buffer.from(normalizedBase64, 'base64');
  ...
  return {
    fileName: sizeBytes > 0 ? fileName : '',
    sizeBytes: sizeBytes > 0 ? sizeBytes : 0,
    uploadedAt: uploadedAtMs > 0 ? new Date(uploadedAtMs).toISOString() : '',
    sha256: String(source.sha256 || '').trim().toLowerCase().slice(0, 128),
    dataBase64: sizeBytes > 0 ? normalizedBase64 : ''
  };
}

function windowsAgentPackageForClient(config = {}) {
  const normalized = normalizeWindowsAgentPackageSettings(config || {});
  return {
    available: Boolean(normalized.sizeBytes > 0 && normalized.dataBase64),
    fileName: normalized.fileName || '',
    sizeBytes: Number(normalized.sizeBytes || 0),
    uploadedAt: normalized.uploadedAt || '',
    sha256: normalized.sha256 || ''
  };
}

async function loadWindowsAgentPackageSettings() {
  try {
    const raw = await smartReadFile(WINDOWS_AGENT_PACKAGE_FILE, 'utf8');
    const parsed = JSON.parse(raw || '{}');
    const normalized = normalizeWindowsAgentPackageSettings(parsed || {});
    setConfigIntegrityState('windowsAgentPackage', 'ok', 'Loaded successfully');
    return normalized;
  } catch (err) {
    const detail = String(err?.message || err || 'Windows agent package settings load failed').trim() || 'Windows agent package settings load failed';
    console.warn(`Windows agent package settings load failed; using defaults. (${detail})`);
    setConfigIntegrityState('windowsAgentPackage', 'fail', detail);
    return { ...defaultWindowsAgentPackageSettings };
  }
}

async function persistWindowsAgentPackageSettings(config = {}) {
  const payload = normalizeWindowsAgentPackageSettings(config || {});
  await smartWriteFile(WINDOWS_AGENT_PACKAGE_FILE, JSON.stringify(payload, null, 2), 'utf8');
  setConfigIntegrityState('windowsAgentPackage', 'ok', 'Saved successfully');
}
...
function sanitizeLocationSettings(config = {}) {
  ...
  if (!sections.length) {
    ...
    sections.push({ id: 'customer', name: clean(config.customerName, defaultLocationSettings.customerName), address: '', pingMonitors: [] });
  }
  ...
  const customerSection = sections[1] || { id: 'customer', name: defaultLocationSettings.customerName };
  return {
    ...
    customerName: requestedCustomer || customerSection.name || defaultLocationSettings.customerName,
    sections
  };
}
